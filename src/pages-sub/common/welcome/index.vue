...
const { needBind, userInfo } = storeToRefs(userStore)
...
async function selectIdentity(identity: IdentityOption) {
  const { id, code: phoneCode } = identity
  ...
  /* // 保存选择的身份到本地存储
  uni.setStorageSync('selectedIdentity', identity.id)

  // 根据身份跳转到不同页面
  if (identity.id === 'parent') {
    // 家长跳转到家长首页
    uni.switchTab({
      url: identity.route!,
    })
    ...
    <view min-h="screen" bg="bg-secondary">
      ...
      >
        ...
        <view relative z="10" pt="16" px="6" text="white">
          ...
          <view text="center" m="b-6">
            <view text="3xl left" font="bold" m="b-3" class="welcome-title">欢迎使用</view>
            <view text="lg left" opacity="75" m="b-2" class="app-name">鑫智生活服务</view>
          </view>
        </view>
      </view>
      ...
      <view px="6" relative z="10" style="margin-top: -32px">
        ...
        <view m="t-8" text="center">
          <view text="xs" color="text-muted">首次使用需要选择身份并进行验证</view>
          <view text="xs" color="text-muted" m="t-1">一个账号可以拥有多种身份</view>
        </view>
      </view>
    </view>
  </Page>
